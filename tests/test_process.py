...
class Test:
    ...
    def test_typecast(self):
        ...
        assert_equal({"float": False}, next(pr.type_cast(records, types)))
    ...
    def test_detect_types(self):
        ...
        assert_equal(Decimal("0.95"), result["confidence"])
        assert_true(result["accurate"])
        ...
        assert_equal(expected, {r["id"]: r["type"] for r in result["types"]})
        assert_equal(record, next(records))

        result = pr.detect_types(records, 0.99)[1]
        assert_equal(100, result["count"])
        assert_equal(Decimal("0.97"), result["confidence"])
        assert_false(result["accurate"])

        result = pr.detect_types([record, record])[1]
        assert_equal(2, result["count"])
        assert_equal(Decimal("0.87"), result["confidence"])
        assert_false(result["accurate"])

    def test_detect_types_datetimes_midnight(self):
        records = it.repeat({"foo": "2000-01-01 00:00:00"})
        records, result = pr.detect_types(records)
        assert_equal(result["types"], [{"id": "foo", "type": "datetime"}])
    ...
    def test_fillempty(self):
        ...
        fields = ["a"]
        assert_equal(values, list(pr.fillempty(records, 0, fields=fields)))

        filled = pr.fillempty(records, method="front")
        assert_equal(more_values_1, list(filled))
    ...
    def test_cut(self):
        ...
        assert_equal({"field_2": "bill"}, result)
    ...
    def test_pivot(self):
        ...
        assert_equal(expected_set, result_set)
